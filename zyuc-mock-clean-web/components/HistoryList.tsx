...
import { fetcher, debounce } from '../lib/utils'; // escapeHtml is no longer needed here
...
const HistoryList = () => {
    ...
    const { data: historyData, error: historyError } = useSWR<HistoryResponse>(
        `/api/history?page=${page}&pageSize=20&project=${encodeURIComponent(projectFilter)}&search=${encodeURIComponent(debouncedSearchTerm)}`,
        fetcher,
    ...
    if (historyError || configsError) return <div>加载历史记录失败...</div>;
    if (!historyData || !configs) return <div>正在加载...</div>;
    ...
    return (
        <div className="list-section">
            ...
            <table id="history-table">
                 <thead>
                    <tr>
                        <th style={{width: '25%'}}>接口 / 工程</th>
                        <th style={{width: '30%'}}>请求内容 (Payload)</th>
                        <th style={{width: '30%'}}>响应内容 (Response)</th>
                    ...
                    {data && data.length > 0 ? (
                        data.map((event, index) => (
                            <tr key={index}>
                                ...
                                {/* --- FIX START: Removed escapeHtml function --- */}
                        ...
                        <tr><td colSpan={4} style={{ textAlign: 'center' }}>未找到历史记录。</td></tr>
                    )}
                </tbody>
            </table>
