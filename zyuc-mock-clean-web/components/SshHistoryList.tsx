...
interface SshHistoryResponse {
    data: SshEventHistory[];
    total: number;
    page: number;
    pageSize: number;
}

const SshHistoryList = () => {
    ...
    const { data: historyData, error: historyError } = useSWR<SshHistoryResponse>(
        `/api/ssh/history?page=${page}&pageSize=20&project=${encodeURIComponent(projectFilter)}&search=${encodeURIComponent(debouncedSearchTerm)}`,
        fetcher,
        { keepPreviousData: true }
    );

    const { data: configs, error: configsError } = useSWR<any[]>('/api/ssh/configs', fetcher);
    ...
    const isLoading = !historyData || !configs;
    const hasError = historyError || configsError;

    if (hasError) return <div>加载SSH历史记录失败...</div>;
    ...
    return (
        <div className="list-section">
            ...
            <table id="history-table">
                <thead>
                <tr>
                    <th style={{width: '25%'}}>命令 / 工程</th>
                    <th style={{width: '40%'}}>响应内容 (Response)</th>
                ...
                {data && data.length > 0 ? (
                    ...
                    <tr><td colSpan={3} style={{ textAlign: 'center' }}>未找到历史记录。</td></tr>
                )}
                </tbody>
            </table>
